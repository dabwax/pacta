<div class="actions columns large-2 medium-3">
    <h3><?= __('Actions') ?></h3>
    <ul class="side-nav">
        <li><?= $this->Html->link(__('New Page Block'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Pages'), ['controller' => 'Pages', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Page'), ['controller' => 'Pages', 'action' => 'add']) ?></li>
    </ul>
</div>
<div class="pageBlocks index large-10 medium-9 columns">
    <table cellpadding="0" cellspacing="0">
    <thead>
        <tr>
            <th><?= $this->Paginator->sort('id') ?></th>
            <th><?= $this->Paginator->sort('name') ?></th>
            <th><?= $this->Paginator->sort('page_id') ?></th>
            <th><?= $this->Paginator->sort('position') ?></th>
            <th><?= $this->Paginator->sort('created') ?></th>
            <th class="actions"><?= __('Actions') ?></th>
        </tr>
    </thead>
    <tbody>
    <?php foreach ($pageBlocks as $pageBlock): ?>
        <tr>
            <td><?= $this->Number->format($pageBlock->id) ?></td>
            <td><?= h($pageBlock->name) ?></td>
            <td>
                <?= $pageBlock->has('page') ? $this->Html->link($pageBlock->page->name, ['controller' => 'Pages', 'action' => 'view', $pageBlock->page->id]) : '' ?>
            </td>
            <td><?= $this->Number->format($pageBlock->position) ?></td>
            <td><?= h($pageBlock->created) ?></td>
            <td class="actions">
                <?= $this->Html->link(__('View'), ['action' => 'view', $pageBlock->id]) ?>
                <?= $this->Html->link(__('Edit'), ['action' => 'edit', $pageBlock->id]) ?>
                <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $pageBlock->id], ['confirm' => __('Are you sure you want to delete # {0}?', $pageBlock->id)]) ?>
            </td>
        </tr>

    <?php endforeach; ?>
    </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
        </ul>
        <p><?= $this->Paginator->counter() ?></p>
    </div>
</div>
